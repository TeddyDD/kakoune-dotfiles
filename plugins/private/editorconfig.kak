...
define-command -override editorconfig-load -params ..1 -docstring "editorconfig-load [file]: set formatting behavior according to editorconfig" %{
    ...
    hook buffer BufWritePre %val{buffile} -group editorconfig-hooks %{ evaluate-commands %sh{
        if [ ${kak_opt_editorconfig_insert_final_newline} = "true" ]; then
            printf %s\\n "try %{ execute-keys -draft %{ gex<a-k>^$<ret> } } catch %{ execute-keys -draft %{ geo<esc> } }"
        fi
